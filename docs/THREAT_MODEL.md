...
- **Deletions and renames are not detected either**, though for a different reason: the library delivers them, and `file.go`'s loop handles only `Write` and `Create`. Verified live: `rm` of a watched `.pem` and `mv` of one out of a watched directory each produced zero events. An attacker who exfiltrates by moving a keystore out of the watched tree leaves no file event.
...
**Detect:** any event at or above `alerter.min_severity` (default `high`) is pushed to every configured channel within roughly a second of the collector observing it. Measured against HEAD in the isolated eval stack: `keystore_write` 87.8 ms median / 282.5 ms p95; `env_write` 53.9 ms / 60.4 ms; `suspicious_outbound` 623.1 ms / 1013.0 ms, the last at `poll_interval: 1s` and meaningless without that qualifier.
...
- Its entire tool surface is five read-only MCP queries. It has no write tool, no shell, no filesystem access, and no ability to change daemon config, suppression rules or alert routing.
...
- **A clean scan posts an affirmative "no threats detected" message.** That statement is only as good as the window it saw, and the window has all the limits above.
...
- **The shipped `mcp_transport: stdio` default does not survive systemd, and the failure is silent.** `ServeStdio` blocks on stdin and returns at EOF; systemd's `StandardInput=` defaults to `null`, "i.e. all read attempts by the process will result in immediate EOF", and `deploy/vigilo.service` does not override it. Measured on HEAD: with stdin on `/dev/null` the daemon logs `vigilo daemon ready`, then `vigilo daemon stopped`, and **exits 0 after ~57 ms**; with stdin held open it runs normally. Reproduced in the shipped container image with `config.example.yaml`. Because the exit status is 0 and the final log line reads like a clean shutdown, `Restart=always` / `RestartSec=5` turns this into a permanent restart loop in which the pollers (default 5s interval) never reach their first tick. `deploy/install.sh` copies `config.example.yaml` — which sets `mcp_transport: stdio` — verbatim to `/etc/vigilo/config.yaml`, so this is the documented install path. Setting `mcp_transport: http` (with a token) avoids it, which is also why no test catches it: the e2e suite and the eval stack both use `http`. **Operators following the README's quick start should assume this applies to them until it is fixed.**
...
- **Killing the daemon is undetectable from inside vigilo.** No heartbeat, no dead-man's switch. Combined with the stdio/systemd behaviour above, "vigilo is installed and enabled" is not equivalent to "vigilo is watching", and nothing in the system distinguishes the two.
- **The event trail is not tamper-evident.** No signing, no hash chaining. Anyone with write access can alter history silently.
- **Three independent evasion paths against file detection have been reproduced live**: reads are invisible to the default collector, deletes and renames are dropped by the collector's own filter, and directories created more than one level deep after startup are never watched.
...
- **Alert suppression is a two-edged control.** Unanchored substring suppression rules can be satisfied by an attacker-chosen path (reproduced), and the default one-hour push cooldown on `source + resource` collapses the repeated-access pattern that most distinguishes extraction from routine work (reproduced).
- **The analyst tier has a small, displaceable window and no injection defence**, and no measured evaluation of any kind. Its containment is structural — read-only tools, Slack-post-only authority, no ability to touch tier 1 — not behavioural.
...
- **The false-positive figure is 0, measured over a 60-second bounded window of in-container dev activity, with no suppression tuning.** The harness itself refuses to report a vacuous zero and carries the qualifier in its output; repeating the number without the qualifier would be dishonest. It is not a production signer's noise profile, and no long-window measurement exists.
- **Every number in this repository comes from a controlled harness that triggers its own events.** Vigilo has never been published, never deployed to a production validator, and never run against real adversarial traffic. Throughput under a real event flood has not been measured at all.
