...
**Tier 1 (`internal/alerter`) answers "did something touch a key just now?"** It is a severity threshold over single events — no state, no model, no correlation. Measured against HEAD in the isolated eval stack (`eval/docker/`, reproduced via `eval/scripts/run_evaluation.py`), median / p95 detection-to-alert latency:
...
The network number is dominated by the poll interval and is only meaningful stated alongside it. At the shipped default of `poll_interval: 5s` the same chain's latency scales with that interval; it has not been measured at 5s.
...
- **The cost is measured and small.** 273 B/event marginal, measured after a WAL checkpoint. The checkpoint matters: an earlier measurement of the same thing reported a per-event figure roughly a hundred times larger because it counted WAL churn as stored data. Total daemon overhead measured on the same pass: 0.24% CPU / 11.82 MiB RSS idle, 1.295% CPU / 13.245 MiB under load.
...
**Measuring the wrong thing.** The event-store cost was first reported at roughly 27 KB/event. That was WAL churn counted as stored data. Measured after a checkpoint the marginal cost is 273 B/event. Both numbers came from the same code; only the measurement was wrong, which is the failure mode that makes benchmark numbers worth distrusting by default.
...
**`mcp_transport: stdio` is the shipped default and does not survive being run as a service.** `ServeStdio` blocks on stdin and returns at EOF, after which the daemon takes its normal shutdown path and exits 0. systemd's `StandardInput=` defaults to `null`, "i.e. all read attempts by the process will result in immediate EOF" (systemd's own `systemd.exec` documentation), and `deploy/vigilo.service` does not override it. Measured on HEAD: with `mcp_transport: stdio` and stdin on `/dev/null`, the daemon exits 0 in ~57 ms; with stdin held open it runs until stdin closes. The shipped container image reproduces the same thing with `config.example.yaml` mounted. Every automated path — the e2e suite, the eval stack — configures `mcp_transport: http`, which is why no test catches it. This is a real gap in the documented install path and is carried in `THREAT_MODEL.md` as an availability failure rather than glossed here.
...
- It has never been run against real adversarial traffic. Every number here comes from a controlled harness that triggers its own events.
