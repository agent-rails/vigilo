...
**Tier 1 (`internal/alerter`) answers "did something touch a key just now?"** It is a severity threshold over single events — no state, no model, no correlation. Measured in the isolated eval stack (`eval/docker/`, reproduced via `eval/scripts/run_evaluation.py`) against implementation commit `d6c9ebc`, n=10 per chain, median / p95 detection-to-alert latency:

| Chain | Median | p95 | Mechanism |
|---|---|---|---|
| `keystore_write` | 83.6 ms | 245.4 ms | fsnotify (event-driven) |
| `env_write` | 26.9 ms | 48.9 ms | fsnotify (event-driven) |
| `suspicious_outbound` | 570.8 ms | 1075.5 ms | `/proc/net/tcp` poll, `poll_interval: 1s` |

The network number is dominated by the poll interval and is only meaningful stated alongside it. At the shipped default of `poll_interval: 5s` the same chain's latency may scale with that interval; it has not been measured at 5s. The file triggers measure writes only; create/remove/rename latency is not represented. The report discloses the 60-second benign-activity window and retains raw JSON outside Git.
...
- **The cost is measured, with coarse resolution.** The release-candidate run measured 137 +/- 137 B/event after a WAL checkpoint: one 4096-byte page divided by 30 events. The estimate is at page resolution and should be treated as an order of magnitude, not a byte count. An earlier measurement reported a per-event figure roughly a hundred times larger because it counted WAL churn as retained data. Container overhead in this run was 0.18% CPU / 13.94 MiB idle and 1.85% CPU / 15.44 MiB under load; those digits carry less precision than they suggest, for three reasons stated below.
...
**Measuring the wrong thing.** The event-store cost was once reported at roughly 27 KB/event because it counted WAL churn as stored data. Checkpointed measurements corrected that; the current release-candidate estimate is 137 +/- 137 B/event at 30 events. The error bar is one page divided by event count, since SQLite grows in whole 4096-byte pages. A bare `137` would imply precision this measurement cannot carry.
